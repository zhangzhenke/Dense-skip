...
from engine_fg import train_one_epoch, evaluate
...
def get_args_parser():
    parser = argparse.ArgumentParser('MAE fine-tuning for image Segment', add_help=False)
    # 批量大小
    ...
    # Dataset parameters
    #数据
    parser.add_argument('--data_path', default='./dataFenGe', type=str,
                        help='dataset path')
    # 种类
    parser.add_argument('--nb_classes', default=20, type=int,
                        help='number of the classification types')
    # 结果
    parser.add_argument('--output_dir', default='./zzk/5/',
                        help='path where to save, empty for no saving')
    # 日志
    parser.add_argument('--log_dir', default='./zzk/5/',
...
def main(args):
    ...
    dataset_train = VOCSegmentation(args.data_path,
                                    year="2012",
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        match = re.search(r'mean IoU: (\d+\.\d+)', val_info)
        if match:
            mean_iou_value = float(match.group(1))  # 将找到的字符串转换为浮点数
            print(f"The mean IoU value is: {mean_iou_value}")
        else:
            print("Mean IoU value not found.")

        # 只在主进程上进行写操作
        if args.rank in [-1, 0]:
            with open(os.path.join(args.output_dir, "miou.txt"), mode="a", encoding="utf-8") as f:
                f.write(str(epoch) + "  " + str(mean_iou_value) + "\n")

        # 只在主进程上进行写操作
        if args.rank in [-1, 0]:
            with open(os.path.join(args.output_dir, "acc.txt"), mode="a", encoding="utf-8") as f:
                f.write(str(epoch) + "\n" + val_info + "\n\n")


        # 绘图
        plt_val_acc.append(mean_iou_value)
    ...
    output_path = os.path.join(args.output_dir, 'VOC2012_miou.png')
